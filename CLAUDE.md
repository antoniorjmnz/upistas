...
- `uv run upistas run [--lote lote1] [--norma v3] [--carpeta ../caja/facturas] [--limit N] [--sin-sync]` — sincroniza el ERP, lee y decide un lote → `outputs/outcomes.jsonl` (con `--lote lote2`, `outputs/outcomes_lote2.jsonl`; otro nombre con `--salida`). Ver [docs/backend.md](docs/backend.md).
