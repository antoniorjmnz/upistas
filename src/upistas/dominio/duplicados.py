...
def resolver_duplicados(decisiones: list[Decision], facturas: Mapping[str, Factura] | None = None,
    ...
    def bloquear(file_id, regla, detalle, resultado):
        ...
        if any(not c.ok and c.regla in ("R0_lectura", "R3_datos_fiscales", "R6_evaluacion_disponible", "R6_contenido_oculto") for c in actual.comprobaciones):
            resultado = Resultado.ESCALAR
        elif actual.resultado == Resultado.ESCALAR and any(not c.ok and c.regla == "R6_notas" for c in actual.comprobaciones):
            resultado = Resultado.ESCALAR
