...
- **Escaneos tenues que el OCR lee "casi" bien**: en `scan_009`, `scan_012`, `scan_013`, `scan_015`, `scan_016`, `scan_017` y `scan_023` la imagen es correcta y cuadra con el ERP, pero Firecrawl devuelve todos los campos con un dígito cambiado (5, 6 o 9 por 8; la B del NIF por 8; 2026 por 2028). Con un solo lector salían NO_PAGAR firmes o duplicados fantasma (`scan_012` y `scan_016` leídos ambos como PO-0488 cuando son 0488 y 0498). La visión de Helmcode también falla un dígito a veces (`scan_012`: 0468 por 0488). Por eso todo escaneo se lee dos veces y cualquier discrepancia escala; ver ADR-002. Solo `scan_016` y `scan_029` tienen trampa de verdad: IBAN distinto del maestro con nota de "nuevo número de cuenta".
